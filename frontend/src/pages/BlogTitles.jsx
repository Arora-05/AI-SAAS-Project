...
const BlogTitles = () => {
    const blogCategories= [ 'General','Technology',
        'Health', 'Lifestyle', 'Travel', 'Food', 'Education', 'Finance', 'Entertainment', 'Sports' ];
    ...
    text-slate-700'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
            {
                blogCategories.map((item) => (
                    <span onClick={()=> setSelectedCategory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer
                        ${selectedCategory === item ? 'bg-purple-100 text-purple-600' : 'text-gray-500 border-gray-300'}`} key={item}> {item} </span>
                ))
            }
        </div>
        <br/>
        ...
        <div className='flex items-center gap-3'>
            <Hash className='w-5 h-5 text-[#8E37EE]'/>
