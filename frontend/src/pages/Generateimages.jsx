...
const Generateimages = () => {
    const ImageStyle = ['Realistic','Cartoon style','Anime style','Ghibli style','3D style','Portrait style',
        'Fantasy style','Sci-fi style'];
      ...
      text-slate-700'>
          ...
          text-sm rounded-md border border-gray-300' placeholder='Describe your image...' required/>
          ...
          <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
              {
                  ImageStyle.map((item) => (
                      <span onClick={()=> setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer
                          ${selectedStyle === item ? 'bg-green-100 text-green-600' : 'text-gray-500 border-gray-300'}`} key={item}> {item} </span>
                  ))
              }
          </div>
